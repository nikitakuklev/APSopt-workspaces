...
def restore_ops_settings(ops_nuy=None):
    ...
    ops_vals = [
        int(OPS[k]["val"]) if k in ("Y_Drive_freq", "Y_Drive_period") else OPS[k]["val"]
        for k in pv_keys
    ]
...
def clean_bunches(
    clean_ini_bunch_number,
    clean_end_bunch_number,
    keep_ini_bunch_number,
    keep_end_bunch_number,
    nuy,
):
    """
